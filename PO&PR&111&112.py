...
Output_path_Pr =f"F:\RYH\物料相关\每日数据源\{date}\purchase.requisition_OK_{date}_物料计划用.xlsx"
...
Output_path_Po =f"F:\RYH\物料相关\每日数据源\{date}\purchase.order_OK_{date}_物料计划用.xlsx"
...
Output_path_112_物料计划用 = f"F:\RYH\物料相关\每日数据源\{date}\stock.quant_112_OK_{date}_物料计划用.xlsx"
...
def Condition_112(df_112):
    return  (df_112.loc[:,"位置"]!="BLYF")&\
            ...
            (df_112.loc[:,"位置"]!="不良仓/库存/沪试料/E栋丙类库/报废位置/YL-FC112/YL-205-TL")&\
            (~df_112.loc[:,"位置"].str.contains("YP"))&\
...
df_112_Done = df_112_Done[["产品/内部参考",'不带C项目','位置','批次/序列号码','数量','单位']]
...
#112_物料计划用导出
df_112_Done_with_sumif.to_excel(Output_path_112_物料计划用)
print("112_物料计划用Done")



path_111 = f"F:\RYH\物料相关\每日数据源\{date}\stock.quant_111.xlsx"
Output_path_111 = f"F:\RYH\物料相关\每日数据源\{date}\stock.quant_111_OK_{date}_物料计划用.xlsx"
...
def Condition_111(df):
    return (df.loc[:,"位置"]!="FC111")&\
            ...
            (df.loc[:,"位置"]!="不良仓/库存/沪试成报废区/报废位置/TH-SH-03")&\
            (df.loc[:,"位置"]!="不良仓/库存/沪试成报废区/报废位置/TH-SH-05")&\
            (df.loc[:,"位置"]!="不良仓/库存/沪试成报废区/报废位置/TH-SH-C")&\
            (df.loc[:,"可用数"] >=1)         
df_111_Done = df_111.loc[Condition_111,:]
df_111_Done= df_111_Done[["不带C项目","位置","批次/序列号码",'数量','预留数量','可用数']]
#sumif 按照 产品/内部参考 并merge到主表
grouped_111 = df_111_Done.groupby("产品/内部参考")["可用数"].sum()
df_111_Done_with_sumif =pd.merge(df_111_Done,grouped_111,how="left",left_on="产品/内部参考",right_on="产品/内部参考")
df_111_Done_with_sumif.to_excel(Output_path_111)
print("111_Done")
